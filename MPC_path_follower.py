...
class KinMPCPathFollower(Controller):
	...
	def __init__(self, 
		         ...
		         L_F        = 1.6, 		# distance from CoG to front axle (m)
		         L_R        = 1.4, 		# distance from CoG to rear axle (m)
		         V_MIN      = 1.0,    # min/max velocity constraint (m/s)
		         V_MAX      = 70.0,     
		         A_MIN      = -15.0,   # min/max acceleration constraint (m/s^2)
		         A_MAX      =  10.0,     
		         DF_MIN     = -0.30,   # min/max front steer angle constraint (rad)
		         DF_MAX     =  0.30,    
		         A_DOT_MIN  = -10,   # min/max jerk constraint (m/s^3)
		         A_DOT_MAX  =  10,
		         DF_DOT_MIN = -np.pi/2,   # min/max front steer angle rate constraint (rad/s)
		         DF_DOT_MAX =  np.pi/2,
				 BETA_MIN = -np.pi,
				 BETA_MAX = np.pi, 
		         Q = [1., 1., 1., 1.], # weights on x, y, psi, and v.
		         R = [1., 1.]):       # weights on jerk and slew rate (steering angle derivative)
		...
		self._update_initial_condition(0., 0., 0., 0.) #modified such that the ego vehicle is stationary at start (t=0)
		...
		# update reference based on the imported lap time optimized trajectory
		# self._update_reference(data[0:self.N,1],
		# 				data[0:self.N,2], 
		# 				data[0:self.N,3], 
		# 				data[0:self.N,5])					  
	...
	def _add_constraints(self):
		...
		self.opti.subject_to( self.opti.bounded(self.BETA_MIN, casadi.atan( self.L_R / (self.L_F + self.L_R) * casadi.tan(self.df_dv[i]) ), self.BETA_MAX))
